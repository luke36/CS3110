...
let take_5th_tests = "test suite for take_5th" >::: [
  "empty" >:: (fun _ -> assert_equal 0 (take_5th []));
  "less than 5" >:: (fun _ -> assert_equal 0 (take_5th [1; 2; 3]));
  "more than 5" >:: (fun _ -> assert_equal 5 (take_5th [1; 2; 3; 4; 5; 6]));
]

let sort_descend_tests = "test suite for sort_descend" >::: [
  "empty" >:: (fun _ -> assert_equal [] (sort_descend []));
  "multiple" >:: (fun _ -> assert_equal [6; 4; 4; 3; 2; 1] (sort_descend [4; 3; 1; 6; 2; 4]));
]

let list_max_tests = "test list max" >::: [
...
let () = run_test_tt_main product_tests;
